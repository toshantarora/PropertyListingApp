...
import com.example.propertylisting.activities.BoothListActivity;
import com.example.propertylisting.activities.HousingBoardListActivity;
import com.example.propertylisting.activities.KothiListActivity;
import com.example.propertylisting.activities.ScoListActivity;
import com.example.propertylisting.activities.SocietyListActivity;

public class ListPropertyDialog extends DialogFragment
{
    @NonNull
    @Override
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState)
    {
        View view = getActivity().getLayoutInflater().inflate(R.layout.dialog_list_property, null);
    ...
    private void executeIntentOnOptionSelect(int position)
    {
        switch (position)
        {
            case 1: // On select: Society Flat
                Intent sf = new Intent(getActivity(), SocietyListActivity.class);
                startActivity(sf);
                dismiss();
                break;
            case 2: // On select: HousingBoard Flat
                Intent hb = new Intent(getActivity(), HousingBoardListActivity.class);
                startActivity(hb);
                dismiss();
                break;
            case 3: // On select: Kothi
                Intent intent = new Intent(getActivity(), KothiListActivity.class);
                startActivity(intent);
                dismiss();
                break;
            case 4: // On select: Sco
                Intent sco = new Intent(getActivity(), ScoListActivity.class);
                startActivity(sco);
                dismiss();
                break;
            case 5: // On select: Booth
                Intent b = new Intent(getActivity(), BoothListActivity.class);
                startActivity(b);
                dismiss();
                break;
            default:
